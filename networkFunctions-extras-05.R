...
plotSampled = function(x, y = NULL, sample = NULL, ...)
{
  n = length(x);
  if (is.null(sample))
  {
  ...
  plot(x[sample], y = if (is.null(y)) y else y[sample], ...);
}
...
thinnedScatterplot = function(x, y, bins = 100, thinTo = 5, log = "", ...)
{
  n = length(x);
  ...
  for (th in 1:nThin)
  {
    ...
    drop = c(drop, sample(pointsInBin, nBin-thinTo));
  }
  xPlot = x[-drop];
  yPlot = y[-drop];

  plot(xPlot, yPlot, log = log, ...);
}
...
verbosePairs = function(x, mar1 = c(2,2,1,1), names = colnames(x), cex.names = 3, sample = NULL,
                        ...
                        histLog = FALSE, verbose = TRUE, cex = 1, 
...
multiData.applyToSubset = function(multiData, FUN, ..., 
                                   mdaColIndex = NULL, mdaRowIndex = NULL, mdaSimplify = TRUE)
{
  size = checkSets(multiData);
  res = vector(mode = "list", length = size$nSets);
...
multiData.setAttr = function(multiData, attribute, valueList)
{
  size = checkSets(multiData);
  ind = 1;
  for (set in 1:size$nSets)
...
multiData.setColnames = function(multiData, colnames)
{
  size = checkSets(multiData);
  for (set in 1:size$nSets)
    colnames(multiData[[set]]$data) = colnames
...
# FIXME: sometimes it would be helpful to convert which from the index to colnames of x. Consider adding an
                    ...
                    labelCol = "termName", pValueThreshold = 0.05, numericModLabels = TRUE)
{
  ...
  order = order(enrichmentTable[, moduleCol], as.numeric(as.character(enrichmentTable[, pValueCol])) )
  ...
  bestP = as.numeric(as.character(enr.ord[bestIndex, pValueCol]))
...
binarizeCategoricalVar = function(x, minCount = 3, val1 = 1, val2 = 2, nameSep = ".vs.", namePrefix = "",
                                  ...
                                  includeLevelVsAll = FALSE, levelOrder = NULL)
{
  ...
  if (includeLevelVsAll)
    for (v1 in 1:nLevels)
    {
      ...
      names[ind] = spaste(namePrefix, levels[v1]);
      ind = ind+1;
    }
  colnames(out) = names;
  out;
...
fisherTestPvalue = function(x, y, alternative = "two.sided")
{
  keep = !is.na(x) & !is.na(y);
  tab = table(x[keep], y[keep]);
  tryCatch( fisher.test(x[keep], y[keep], alternative = alternative)$p.value, error = function(e) {NA});
}
...
testPvalue = function(contX, binaryY, test = kruskal.test, args = list())
{
  test = match.fun(test);
  contX = as.matrix(contX);
  binaryY = as.matrix(binaryY);
  nVars = ncol(contX);
  nOutcomes = ncol(binaryY);

  p = matrix(NA, nVars, nOutcomes);
  colnames(p) = colnames(binaryY);
  rownames(p) = colnames(contX);
  for (o in 1:nOutcomes)
  {
    y = as.numeric(factor(binaryY[, o]))-1;
    for (v in 1:nVars)
    {
       x = contX[, v];
       keep = is.finite(x) & is.finite(y);
       if (length(unique(y[keep]))==1) 
       {
         ...
         p[v, o] = tryCatch(do.call(test, c(list(formula = x~y), args))$p.value, error = function(e) {NA});
       }
    }
  }
  finite = is.finite(p);
  p[finite][p[finite] > 1] = p;
  p;
}
...
# Multi-column display for a tall, narrow labeled heatmaps.
...
colStatsByGroup = function(x, g = NULL, assignNames = TRUE, getSquares = TRUE, simplify = TRUE)
{
  ...
  for (gl in 1:nLevels)
  {
    indicator = as.numeric(is.finite(g) & g==gLevels[gl]);
  ...
  if (getSquares) {
    groupSD = sqrt( (groupSumSq - groupMean^2 * groupNPresent) / (groupNPresent - 1) );
    groupSD[groupSD < 0] = NA;
  } else {
    groupSumSq = groupSD = NULL;
  }
...
matrixSingleSampleTTest = function(x, alternative, colStats = NULL)
{
  ...
  cbind(statistic = t, parameter = df, p.value = p)
}
...
matrixSingleSampleBinomialTest = function(x, alternative)
{
  ...
  cbind(p.value = p, nLess = nSmaller, nGreater = nLarger);
}
                               ...
                               colStats = NULL)
{
  ...
  if (!any(gFinite))
  {
    out = matrix(NA, nCols, 3);
    colnames(out) = c("statistic", "parameter", "p.value");
    return(out);
  }

  if (is.null(colStats))
    colStats = colStatsByGroup(x, g, assignNames = FALSE, getSquares = TRUE);
  ...
  t = (groupMean[1, ] - groupMean[2, ])/denom;
...
matrixKruskalTest.singleG = function(x, g, getSign, getAreaUnderROC)
{
  ...
  if (!any(gFinite))
  {
    out = matrix(NA, nCols, 3 + getSign + getAreaUnderROC);
...
{
  statNames = c("statistic", "parameter", "p.value");
                       ...
                       var.equal = FALSE)
{
  ...
  matrixTest(x, g, testFnc = matrixTTest.singleG, statNames = basicStatNames,
...
nSSBTstats = function(getQvalues, ...)
{
  15 + 2*as.numeric(getQvalues);
}

SSBTstatNames = function(getQvalues, corName, levelNames = NULL, dataIsLogTransformed, ...)
{
  ...
  c(corName, "t.Student", if (getQvalues) c("p.Student", "q.Student") else "p.Student",
    "foldChange",
    spaste("mean", logMod, ".group.", levelNames), spaste("stdErr", SElogMod, ".group.", levelNames),
    spaste("nPresent.group.", levelNames), "nPresent.bothGroups",
    "stat.Kruskal", "stat.Kruskal.signed", 
    if (getQvalues) c("p.Kruskal", "q.Kruskal") else "p.Kruskal",
    "areaUnderROC")
}

SSSstatNames = function(getQvalues, dataIsLogTransformed, ...)
{
  if (dataIsLogTransformed)
  {
    logMod = "Log"
  ...
  c("t.Student", if (getQvalues) c("p.Student", "q.Student") else "p.Student",
    spaste("mean", logMod), spaste("stdErr", SElogMod), "nPresent",
    if (getQvalues) c("p.Binomial", "q.Binomial") else "p.Binomial",
    "nNegative", "nPositive");
}
...
nSSNTstats = function(getQvalues, getAreaUnderROC, ...)
{
  4 + getQvalues + getAreaUnderROC;
}

SSNTstatNames = function(getQvalues, corName, getAreaUnderROC, ...)
{
  c(corName, "Z", if (getQvalues) c("p.Student", "q.Student") else "p.Student",
        if (getAreaUnderROC) c("areaUnderROC", "nPresent") else "nPresent");
}
           ...
           traitName = "" )
{
  ...
  out = matrix(NA, nVars, nSSBTstats(getQvalues));
  colnames(out) = SSBTstatNames(getQvalues, corName, levelNames, dataIsLogTransformed);
  ...
  tTest = matrixTTest.singleG(data, 1-trait, var.equal = var.equal, alternative = tTestAlternative,
                              colStats = colStats)
  ...
  out[, ind:(ind + 7)] = cbind(ifelse(mean1.t > mean2.t, mean1.t/mean2.t, -mean2.t/mean1.t),
               mean1, mean2, t(colStats$stdDev/sqrt(colStats$nPresent)),
               t(colStats$nPresent), colSums(colStats$nPresent));
  ind = ind + 8;

  # Kruskal test (including area under ROC)
  kruskalTest = matrixKruskalTest.singleG(data, -trait, getSign = TRUE, getAreaUnderROC = TRUE);
  ...
  out[, ind] = 1-kruskalTest[, "areaUnderROC"];

  out;
}

# Workhorse function for single-sample screening
                            ...
                            traitName = "")
{
  ...
  colnames(output) = SSNTstatNames(getQvalues = getQvalues, corName = corName, 
           ...
           additionalVarInfo = NULL)
{
  ...
  if (length(d) > 1)
  {
     ...
     if (flatOutput)
     {
       dim(out) = c(nVars, nStats *d[2]);
       colnames(out) = spaste(statNames, statTraitNameSep, rep( traitNames, rep(nStats, nTraits)));
           ...
           additionalVarInfo = NULL)
{
  ...
  standardScreening.generic(data, trait, 
               ...
               statTraitNameSep = statTraitNameSep,
                        ...
                        doCollectGarbage = FALSE)
{
  ...
  if (is.null(varNames)) varNames = varNames(multiExpr);
  if (is.null(varNames)) varNames = spaste("Variable.", prependZeros(1:nVars))
  ...
  if (tSize$nGenes > 1)
  {
    ...
    if (is.null(traitNames)) traitNames = varNames(multiTrait);
    ...
    for (t in 1:nTraits)
    {
       ...
       mTrait1 = multiData.subset(multiTrait, colIndex = t)
       if (doCollectGarbage) collectGarbage();
  ...
  for (set in 1:size$nSets)
  {
    if (binary)
    {
      setResults[[set]] = SSBT.singleTrait(multiExpr[[set]]$data,
                            ...
                            corFnc = corFnc, corOptions = corOptions, corName = corName,
                            dataIsLogTransformed = dataIsLogTransformed,
                            traitName = varNames(multiTrait[[set]]$data));
      trafo = TRUE;
      if (metaKruskal) 
      {
  ...
  if (trafo)
  {
    ...
    if (length(pCols)!=nSets)
      stop(spaste("Columns for meta-analysis are not unique. This could be due to an internal error\n",
                "  or because a trait or set name matches one of the names of the meta-analysis \n",
                "  statistics. To facilitate solving this problem, here is the relevant information:\n",
                "  meta statistic search pattern: ^", metaP, "\n",
                "  Column names of the combined results in which meta statistic is searched for: \n",
                fixLabels(paste( colnames(comb), collapse = ", "), maxCharPerLine = 60, split = " ")));
...
signifNumeric = function(x, digits)
{
  x = as.data.frame(x);
  isNumeric = sapply(x, is.numeric);
  x[, isNumeric] = signif(x[, isNumeric], digits);
  x;
}
...
dataForTable = function(x, transpose = TRUE, IDcolName = "ID")
{
  if (transpose) out = t(x) else out = x;
  out = data.frame(ID = rownames(out), out);
                      ...
                      ...)
{
  ...
  if (is.null(xlim)) xlim = range(sapply(x, range, na.rm = TRUE), na.rm = TRUE)
  if (is.null(ylim)) ylim = range(sapply(y, range, na.rm = TRUE), na.rm = TRUE)

  plot(x[[1]], y[[1]], xlim = xlim, ylim = ylim, type = type, pch = pch[1], col = col[1],
       bg = bg[[1]], lwd = lwd[1], lty = lty[1], cex = cex[1], ...);
  ...
  for (p in 2:nx)
  {
    ...
    if (type %in% c("l", "b"))
      lines(x[[p]], y[[p]], col = col[p], lwd = lwd[p], lty = lty[p], cex = cex[p]);
  }
}
                            ...
                            maxLegendLength = 60, useLabels2colors = TRUE, ...)
{
  ...
  if (is.null(ordinalColumns))
  {
    ...
    ordinalColumns = nUnique <= maxOrdinalLevels;
  ...
  {
    x1 = x[[c]];
    unique1 = sort(unique(x1));
    if (ordinalColumns[c])
    {
      fac = as.numeric(factor(x1));
      if (useLabels2colors) 
      {
  ...
  list(colors = colors, legend = finalLegend, 
       allLegends = cbind(long = legend.long, standard = legend.std, short = legend.short));
}
